...
-- uninstall: exec drop_table('stat_calc_type');
begin 
...
begin 
   ...
   add_pk_constraint('stat_profile', 'stat_profile_id');
end;
/

/*

Buckets are used to group a set of metrics.

The bucket key must be unique across all buckets.

The user_id is optional, can be used for anything, but typically links to a row in the saas_auth table.
...
begin 
   if not does_table_exist('stat_bucket') then
      ...
      -- Unique string developer uses as a known reference to a bucket.
      bucket_key varchar2(250) not null,
      bucket_token varchar2(250) not null,
      -- Friendly name for the bucket. 
      ...
      -- ToDo: Change to default_calc_type
      ...
      ignore_negative number default 0 not null,
   ...
   add_pk_constraint('stat_bucket', 'bucket_id');
   if not does_index_exist('stat_bucket_1') then 
      execute_sql('create unique index stat_bucket_1 on stat_bucket (bucket_key)', false);
   end if;
   if not does_constraint_exist('stat_bucket_fk_calc_type') then
   ...
   if not does_constraint_exist('stat_bucket_fk_user_id') then 
      execute_sql('alter table stat_bucket add constraint stat_bucket_fk_user_id foreign key (user_id) references saas_auth (user_id) on delete cascade', false);
   end if;
end;
/
...
begin 
   if not does_table_exist('stat') then
      execute_sql('
         ...
         stat_key varchar2(250) not null,
   ...
   add_pk_constraint('stat', 'stat_id');
   if not does_constraint_exist('stat_bucket_id_fk') then
      execute_sql('alter table stat add constraint stat_bucket_id_fk foreign key (bucket_id) references stat_bucket (bucket_id) on delete cascade', false);
   end if;
end;
/
...
begin 
   ...
   add_pk_constraint('stat_property', 'stat_property_id');
   if not does_constraint_exist('stat_property_stat_id_fk') then
      execute_sql('alter table stat_property add constraint stat_property_stat_id_fk foreign key (stat_id) references stat(stat_id)', false);
   end if;
end;
/
...
Stat in recieves data.

The bucket_key must map to a bucket.

stat_key must be unique within a bucket.

stat_name does not need to be unique within a bucket.
   For example, we may have "CPU Usage" >1 times within a bucket.
   We can use stat_description to differentiate these values if desired.
   stat_key also differentiates values.
...
begin 
   if not does_table_exist('stat_in') then
      ...
      stat_key varchar2(250) not null,
      stat_name varchar2(250) not null,
      stat_description varchar2(250) default null,
      static_json varchar2(250) default null,
      dynamic_json varchar2(250) default null,
      stat_time timestamp default systimestamp,
      received_val number not null,
      created timestamp default systimestamp)', false);
   end if;
   add_pk_constraint('stat_in', 'stat_in_id');
   if not does_index_exist('stat_in_1') then 
      execute_sql('create unique index stat_in_1 on stat_in (stat_key, bucket_key, stat_time)', false);
   end if;
end;
/
...
begin 
   if not does_table_exist('stat_work') then
      ...
      stat_work_id number generated by default on null as identity cache 20 noorder nocycle nokeep noscale not null,
      stat_key varchar2(250) not null,
      stat_name varchar2(250) not null,
      stat_description varchar2(250) default null,
   ...
   add_pk_constraint('stat_work', 'stat_work_id');
   ...
   if not does_index_exist('stat_work_1') then 
      execute_sql('create unique index stat_work_1 on stat_work (stat_key, bucket_id)', false);
   end if;
end;
/
...
begin 
   ...
   if not does_index_exist('stat_archive_1') then 
      execute_sql('create unique index stat_archive_1 on stat_archive (stat_key, stat_time, bucket_id)', false);
   end if;
end;
/
...
begin
   if not does_table_exist('stat_avg_val_hist_ref') then 
      ...
      execute_sql('alter table stat_avg_val_hist_ref add constraint pk_stat_avg_val_hist_ref primary key (avg_val_ref_group, hist_key, bucket_id, stat_key)', false);
   end if;
   if not does_constraint_exist('stat_avg_val_hist_ref_fk_bucket_id') then
      execute_sql('alter table stat_avg_val_hist_ref add constraint stat_avg_val_hist_ref_fk_bucket_id foreign key (bucket_id) references stat_bucket (bucket_id) on delete cascade', false);
   end if;
end;
/
...
begin
   if not does_table_exist('stat_percentiles_ref') then 
      ...
      execute_sql('alter table stat_percentiles_ref add constraint pk_stat_percentiles_ref primary key (bucket_id, stat_key)', false);
   end if;
   if not does_constraint_exist('stat_percentiles_ref_fk_bucket_id') then
      execute_sql('alter table stat_percentiles_ref add constraint stat_percentiles_ref_fk_bucket_id foreign key (bucket_id) references stat_bucket (bucket_id) on delete cascade', false);
   end if;
end;
/

-- uninstall: exec drop_view ('v_stat_avg_val_hist_ref');
...
begin 
   ...
   add_pk_constraint('stat_detail', 'stat_detail_id');

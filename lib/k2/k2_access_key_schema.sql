...
begin 
	if not does_table_exist('access_key') then 
		execute_sql('
	...
	add_pk_constraint('access_key', 'access_key_id');
	if not does_constraint_exist('access_key_fk_user_id') then
		execute_sql('alter table access_key add constraint access_key_fk_user_id foreign key (user_id) references saas_auth (user_id) on delete cascade', false);
	end if;
	if not does_index_exist('access_key_1') then
		execute_sql('create unique index access_key_1 on access_key (access_key_key)', false);
	end if;
end;
/

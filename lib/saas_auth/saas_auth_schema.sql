...
-- uninstall: exec drop_table('saas_auth_role');
begin
   if not does_table_exist('saas_auth_role') then 
      ...
      add_pk_constraint('saas_auth_role', 'role_id');
   end if;
   if not does_index_exist('saas_auth_role_1') then 
...
begin 
   ...
   -- system accounts are for back end use and are not able to log in
   update saas_auth_role set role_id=3 where role_id=3;
   if sql%rowcount = 0 then 
      insert into saas_auth_role (
         role_id,
         role_name) values (
...
begin
   ...
   if is_column_nullable('saas_auth', 'role_id') then 
      execute_sql('alter table saas_auth modify (role_id not null)', false);
   end if;
   if not does_index_exist('saas_auth_2') then 
...
create or replace view v_saas_auth_available_accounts as
   select * 
    ...
    where account_status in ('active', 'inactive')
      ...
      -- This will prevent system role from logging in via the custom_auth procedure.
      -- system accounts are only for backend use and should not be able to log in.
      and role_id not in (select role_id from saas_auth_role where role_name = 'system');

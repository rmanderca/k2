...
create or replace procedure app_send_email ( -- | Primary email interface for sending an email.
   p_to in varchar2, -- | Valid email address to which the email is sent (required). For multiple email addresses, use a comma-separated list
   p_from in varchar2, -- | Email address from which the email is sent (required). This email address must be a valid address. Otherwise, the message is not sent.
   p_body in varchar2, -- | Body of the email in plain text, not HTML (required). If a value is passed to p_body_html, then this is the only text the recipient sees. If a value is not passed to p_body_html, then this text only displays for email clients that do not support HTML or have HTML disabled. A carriage return or line feed (CRLF) must be included every 1000 characters.
   p_subject in varchar2 default null -- | Subject of the email.
   ) is
begin 
...
create or replace procedure app_send_email_attachement ( -- | Primary email interface for sending an email with an attachment.
   p_to in varchar2,
   p_from in varchar2,
   p_body in varchar2, 
...
begin 
   ...
   apex_mail.push_queue;
   arcsql.increment_counter(app_config.app_name||', app_send_email_attachement, '||p_to);
   commit;
end;
/
